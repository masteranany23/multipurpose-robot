...
class Face_Recognizer:
    ...
    def init_camera(self):
        ...
        self.picam2.start()
        time.sleep(2)  # Allow camera to warm up and adjust
    ...
    #  Face detection and recognition with picamera2
    def process(self):
        # 1.  Get faces known from "features.all.csv"
        if self.get_face_database():
            self.init_camera()
            
            # Initialize face name list
            self.current_frame_face_name_list = []
            
            while True:
                self.frame_cnt += 1
                logging.debug("Frame " + str(self.frame_cnt) + " starts")
                flag, img_rd = self.get_frame()
                
                if not flag:
                    logging.error("Failed to get frame")
                    continue
                    
                # Convert for dlib processing
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    if self.current_frame_face_name_list and "unknown" in self.current_frame_face_name_list:
                    ...
                    for i in range(self.current_frame_face_cnt):
                        ...
                        if i < len(self.current_frame_face_name_list) and i < len(self.current_frame_face_position_list):
                        ...
                        for i in range(len(faces)):
                            try:
                                ...
                                self.current_frame_face_feature_list.append(
                                    face_reco_model.compute_face_descriptor(img_rgb, shape))
                                self.current_frame_face_name_list.append("unknown")
                            except Exception as e:
                                logging.error(f"Error in face feature extraction: {e}")
                                if len(self.current_frame_face_name_list) <= i:
                                    self.current_frame_face_name_list.append("unknown")

                        # 6.2.2.1 Traversal all the faces in the database
                        for k in range(len(faces)):
                            if k >= len(self.current_frame_face_feature_list):
