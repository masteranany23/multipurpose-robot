...
class Face_Register:
    def __init__(self):
        ...
        # Initialize Picamera2 instead of OpenCV VideoCapture
        try:
            self.picam2 = Picamera2()
            # Configure the camera with a reasonable resolution
    ...
    def check_existing_faces_cnt(self):
        if os.path.exists("data/data_faces_from_camera/") and os.listdir("data/data_faces_from_camera/"):
            # Get the order of latest person
            person_list = os.listdir("data/data_faces_from_camera/")
            person_num_list = []
            for person in person_list:
                person_parts = person.split('_')
                if len(person_parts) > 1:
                    try:
                        person_order = person_parts[1]
    ...
    def create_face_folder(self):
        ...
        os.makedirs(os.path.dirname(self.current_face_dir), exist_ok=True)
                ...
                if not self.out_of_range_flag:
                    ...
                    for ii in range(self.face_ROI_height * 2):
                        for jj in range(self.face_ROI_width * 2):
                            if (self.face_ROI_height_start - self.hh + ii < self.current_frame.shape[0] and
                                self.face_ROI_width_start - self.ww + jj < self.current_frame.shape[1] and
                                self.face_ROI_height_start - self.hh + ii >= 0 and
                                self.face_ROI_width_start - self.ww + jj >= 0):
                    ...
                    # Ensure BGR format for OpenCV image writing
                    if len(self.face_ROI_image.shape) == 3 and self.face_ROI_image.shape[2] == 3:
                        save_image = cv2.cvtColor(self.face_ROI_image, cv2.COLOR_RGB2BGR)
                    else:
                        save_image = self.face_ROI_image
                        
                    cv2.imwrite(self.current_face_dir + "/img_face_" + str(self.ss_cnt) + ".jpg", save_image)
    ...
    def get_frame(self):
        # Use picamera2 instead of OpenCV's VideoCapture
        ...
        # Return a blank frame if camera access fails
        ...
        if ret:
            ...
            if len(faces) != 0:
                ...
                for k, d in enumerate(faces):
                    ...
                    # If the size of ROI > frame dimensions
                    if (d.right() + self.ww) > self.current_frame.shape[1] or (d.bottom() + self.hh) > self.current_frame.shape[0] or (d.left() - self.ww < 0) or (d.top() - self.hh < 0):
    ...
    def __del__(self):
        # Clean up camera resources when object is destroyed
